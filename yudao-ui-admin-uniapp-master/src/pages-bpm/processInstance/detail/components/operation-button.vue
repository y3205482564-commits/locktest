...
import { setPendingFormVariables, setPendingTaskForm } from '../process-instance-form-store'
...
/** 从流程表单可编辑字段收集变量（审批通过时随请求提交） */
const props = withDefaults(
  defineProps<{
    getFormVariables?: () => Record<string, any>
  }>(),
  { getFormVariables: undefined }
)
...
function handleOperation(operationType: number) {
  switch (operationType) {
    case BpmTaskOperationButtonTypeEnum.APPROVE:
      // 审批通过前保存流程表单可编辑字段的值，供审批页提交时带上
      const variables = props.getFormVariables?.() ?? {}
      setPendingFormVariables(variables)
      // 保存当前节点的办理/审批表单配置，审批页内直接展示（与 PC 端「填写表单【formName】」一致）
      if (runningTask.value?.formId != null && runningTask.value?.formConf) {
        setPendingTaskForm({
          formName: runningTask.value.formName,
