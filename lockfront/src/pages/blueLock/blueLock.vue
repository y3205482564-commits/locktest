...
<script setup>
	...
	const user = uni.getStorageSync('user') || { userId: 'mock_user_001' };
	...
	function showPopupMessage(type, text) {
		...
		console.log(msgType.value, messageText.value)
		...
		uni.getBLEDeviceServices({
			...
			success(res) {
				...
				if (res.services && res.services.length > 0) {
					...
					// res.services.slice(0, 1).forEach((service) => {
					// 	if (service.isPrimary) {
					// 		blueServiceId.value = service.uuid;
