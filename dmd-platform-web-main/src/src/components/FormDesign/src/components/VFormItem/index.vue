...
const formItemProps = computed(() => {
  ...
  if (!itemProps?.labelCol?.span) newConfig.labelCol = labelCol;

  if (!itemProps?.wrapperCol?.span) newConfig.wrapperCol = wrapperCol;

  if (!itemProps?.rules) newConfig.rules = rules;
...
const cmpProps = computed(() => {
  const isCheck =
    props.schema &&
    ["Switch", "Checkbox", "Radio"].includes(props.schema.component);
...
const handleChange = function (e) {
  const isCheck = ["Switch", "Checkbox", "Radio"].includes(
...
<template>
  <Col v-bind="colPropsComputed">
    <FormItem v-bind="{ ...formItemProps }">
      <template
        v-if="!formItemProps.hiddenLabel && schema.component !== 'Divider'"
        #label
      >
        <Tooltip>
      ...
      <span v-if="['Button'].includes(schema.component)">{{
        schema.label
      }}</span>
    </FormItem>
  </Col>
