      ...
      dictOption.push({
        ...
        value:
          valueType === 'string'
            ...
            : valueType === 'boolean'
              ? `${dict.value}` === 'true'
              : valueType === 'number' && !isNaN(Number.parseFloat(`${dict.value}`))
...
export function getDictLabel(dictType: string, value: any) {
  const dictObj = dictStore.getDictMap[dictType];
  if (dictObj.length > 0) {
    return dictObj.find((v:any) => v.value === value.toString())?.label;
  }
  return null
}
...
export enum DICT_TYPE {
  ...
  // 其他字典...
  REPAIR_AUDIT_STATUS = 'repair_audit_status', // 检修单审核状态
  REPAIR_ORDER_STATUS = 'repair_order_status'  // 检修单状态
