...
import { BpmAutoApproveType, BpmModelFormType, BpmModelType } from '@/enums';
...
interface BpmProcessDefinitionType {
  id?: string
  type?: number
  simpleModel?: any
  [key: string]: any
...
async function initData() {
  ...
  if (actionType === 'definition') {
    ...
    formData.value.managerUserIds.push(userStore.getUserInfo?.user?.id);
  }
...
<template>
  <div class="mx-auto">
    <!-- 头部导航栏 -->
    <div
      class="absolute inset-x-0 top-0 z-10 h-12 flex items-center border-b bg-white px-5"
    >
      <!-- 左侧标题 -->
      ...
      <div class="h-full flex flex-1 items-center justify-center">
        <div class="h-full w-auto flex items-center justify-center">
          <div
            ...
            :key="index"
            class="relative mx-6 h-full flex cursor-pointer items-center"
            :class="[
              currentStep === index
          ...
          >
            <div
              class="mr-2 h-7 w-7 flex items-center justify-center border-2 rounded-full border-solid text-base"
              :class="[
                currentStep === index
    ...
    <Card :body-style="{ padding: '10px' }" class="mb-4">
      <div class="mt-12">
        ...
        <div v-if="currentStep === 3" class="mx-auto w-4/6">
          <ExtraSetting ref="extraSettingRef" v-model="formData" />
        </div>
      </div>
    </Card>
  </div>
