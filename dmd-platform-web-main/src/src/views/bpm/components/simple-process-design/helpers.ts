...
export interface UserTaskFormType {
...
export interface CopyTaskFormType {
  candidateStrategy: CandidateStrategy
  deptIds?: number[] // 部门
  deptLevel?: number // 部门层级
  expression?: string // 流程表达式
...
export function useNodeForm(nodeType: BpmNodeTypeEnum) {
  ...
  const userOptions = inject<Ref<any[]>>('userList', ref([])); // 用户列表
  const deptOptions = inject<Ref<any[]>>('deptList', ref([])); // 部门列表
  const userGroupOptions = inject<Ref<BpmUserGroupApi.UserGroup[]>>(
    'userGroupList',
    ref([]),
  ); // 用户组列表
  const deptTreeOptions = inject<Ref<any[]>>(
    'deptTree',
    ref([]),
  ); // 部门树
  const formFields = inject<Ref<string[]>>('formFields', ref([])); // 流程表单字段
