...
const globalErrorNodes = ref([]);

const validate = async () => {
  const errorNodes: SimpleFlowNode[] = [];
  validateNode(processNodeTree.value, errorNodes);
  globalErrorNodes.value = errorNodes;
...
<template>
  <div v-loading="loading">
    ...
    />
    <basic-modal title="流程设计校验不通过" class="w-2/5" @register="registerModal">
      <div class="mb-2 text-base">
        以下节点配置不完善，请修改相关配置
      </div>
      <div
        v-for="(item, index) in globalErrorNodes"
        :key="index"
