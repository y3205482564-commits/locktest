...
const [registerDrawer, { closeDrawer, setDrawerProps }] = useDrawerInner(() => {
  // 在打开时不做任何操作，由 showUserTaskNodeConfig 方法处理
});
...
defineExpose({ showUserTaskNodeConfig, registerDrawer }); // 暴露方法给父组件
...
<template>
  <BasicDrawer width="40%" :show-footer="true" @register="registerDrawer" @ok="saveConfig">
    <template #title>
      <div class="config-header">
        ...
        <div v-else class="node-name">
          {{ nodeName }}
          <Icon class="ml-1" icon="ant-design:edit-outlined" @click="clickIcon()" />
        </div>
      </div>
    </template>
    <div
    ...
    >
      <TabPane key="user" :tab="`${nodeTypeName}人`">
        <div>
          <Form
            ref="formRef"
            :model="configForm"
            ...
            <Row class="border border-t-0 border-gray-200 px-4 py-2">
              ...
              <Col :span="12" class="flex items-center">
                ...
                <Button v-else text @click="changeBtnDisplayName(index)">
                  <div class="flex items-center">
                    {{ item.displayName }}
                    <Icon icon="ant-design:edit-outlined" class="ml-2" />
                  </div>
                </Button>
              </Col>
      ...
      <TabPane key="listener" tab="监听器" :force-render="true">
        <UserTaskListener
