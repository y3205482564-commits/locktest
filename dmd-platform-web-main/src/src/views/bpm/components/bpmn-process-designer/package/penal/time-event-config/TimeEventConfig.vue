...
const helpText = computed<string>(() => {
  ...
  if (type.value === 'duration') {
    return 'ISO 8601格式，如PT1H';
  }
  if (type.value === 'cycle') {
    return 'CRON表达式或ISO 8601周期';
  }
  return '';
});
...
function updateNode(): void {
  ...
  if (!moddle || !modeling || !elementRegistry) {
    return;
  }

  // 获取元素
  if (!props.businessObject || !props.businessObject.id) {
    return;
  }
  ...
  <div class="panel-tab__content">
    ...
    >
      ...
      <template #footer>
        ...
        <Button type="primary" @click="onDateConfirm">
          确定
        </Button>
      </template>
    </Modal>
    ...
    >
      ...
      <template #footer>
        <Button @click="showDurationDialog = false">
          取消
        </Button>
        <Button type="primary" @click="onDurationConfirm">
          确定
        </Button>
      </template>
    </Modal>
    ...
    >
      ...
      <template #footer>
        <Button @click="showCycleDialog = false">
          取消
        </Button>
        <Button type="primary" @click="onCycleConfirm">
          确定
        </Button>
      </template>
    </Modal>
