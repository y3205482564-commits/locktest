...
const getActiveElement = () => {
  if (!isReady.value || !props.bpmnModeler) {
    return;
  }
...
const initFormOnChanged = (element: any) => {
  if (!isReady.value || !bpmnInstances()) {
    return;
  }
  ...
  try {
    ...
    // 支持从网关（包括排他网关）和开始事件出发的连线显示流转条件
    const sourceType = activatedElement.source?.type as string || '';
    conditionFormVisible.value
      = elementType.value === 'SequenceFlow'
      && activatedElement.source
      && (
        sourceType.includes('StartEvent')
...
<template>
  ...
  >
    <Collapse v-if="isReady" v-model:active-key="activeTab">
      ...
      <CollapsePanel v-if="formVisible" key="form" header="表单">
        <template #extra>
          <Icon icon="ep:list" />
        </template>
