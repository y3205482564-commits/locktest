...
const categoryList = ref<CategoryVO[]>([]); // 分类的列表
...
const selectProcessDefinition = ref<BpmProcessDefinitionApi.ProcessDefinition>(); // 选择的流程定义
  ...
  (newCategories) => {
    if (newCategories.length > 0) {
      ...
      const currentCategoryExists = newCategories.some(
        (category: CategoryVO) => category.code === activeCategory.value,
      );
