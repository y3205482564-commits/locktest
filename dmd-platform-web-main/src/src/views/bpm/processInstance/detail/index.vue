...
// 导入工单详情组件（根据你的实际路径调整）
import WorkOrderDetail from "./modules/work-order-detail.vue"; // 新增：导入工单详情组件
...
const activeTab = ref('form'); // 默认激活的标签页
...
async function getApprovalDetail() {
  ...
  try {
    ...
    // 新增：从流程实例的表单变量中提取internalCode
    internalCode.value = data.processInstance?.formVariables?.internalCode || '';
      ...
      <div class="h-full flex flex-col">
        ...
        <div class="process-tabs-container flex flex-1 flex-col">
          <Tabs v-model:active-key="activeTab" class="mt-0 h-full">
            ...
            <!-- 新增：工单详情 Tab 页签 -->
            <TabPane key="workOrder" tab="工单详情" class="tab-pane-content">
              <div class="h-full p-4">
                <!-- 修改：使用工单详情组件并传递动态获取的internalCode -->
                <WorkOrderDetail
                  ref="workOrderRef"
                  :work-order-id="internalCode"
