...
import { BpmCandidateStrategyEnum, BpmNodeIdEnum } from '@/enums';

import { useTabs } from '@/hooks/web/useTabs';
import { Icon } from '@/components/Icon';

import { Button, Card, message, Space, Modal } from 'ant-design-vue';
...
async function onSubmit() {
  ...
  // 提交表单
  const data = await getFieldsValue() as BpmOALeaveApi.Leave;
  // 审批相关：设置指定审批人
  if (startUserSelectTasks.value?.length > 0) {
