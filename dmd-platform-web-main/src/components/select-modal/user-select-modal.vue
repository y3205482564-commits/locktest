...
// 用户数据管理
const userList = ref<UserVO[]>([]); // 存储所有已知用户
...
// 弹窗配置 - 使用 vben-admin2 的 useModalInner
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  try {
    ...
    deptTree.value = treeData.map((node: any) => processDeptNode(node));
...
async function loadUserData(pageNo: number, pageSize: number) {
  try {
    ...
    const newUsers = list.filter(
      (user: UserVO) => !userList.value.some((u) => u.id === user.id),
    );
