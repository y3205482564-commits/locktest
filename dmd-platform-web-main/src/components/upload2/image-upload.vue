...
import { defaultImageAccepts, isImgTypeByName } from '@/components/Upload/src/helper';
...
async function beforeUpload(file: File) {
  ...
  if (fileList.value!.length >= props.maxNumber) {
    message.error(t('component.upload.maxNumber', [props.maxNumber]));
    return Upload.LIST_IGNORE;
  }

  const { maxSize, accept } = props;
  const isAct = isImgTypeByName(file.name) || accept.some((ext) => file.name.toLowerCase().endsWith(`.${ext.toLowerCase()}`));
  if (!isAct) {
    message.error(t('component.upload.acceptUpload', [accept]) || `只能上传 ${accept.join('/')} 格式文件`);
  ...
  if (isLt) {
    message.error(t('component.upload.maxSizeMultiple', [maxSize]) || `文件大小不能超过 ${maxSize}MB`);
    isLtMsg.value = false;
    // 防止弹出多个错误提示
...
async function customRequest(info: UploadRequestOption<any>) {
  ...
  try {
    ...
    info.onSuccess!(res);
    message.success(t('component.upload.uploadSuccess') || '上传成功');
  } catch (error: any) {
    console.error(error);
    info.onError!(error);
  ...
  <div>
    ...
    >
      ...
      >
        ...
        <div class="mt-2">
          {{ t('component.upload.imgUpload') || '上传图片' }}
        </div>
      </div>
    </Upload>
    <div
    ...
    >
      请上传不超过
      <div class="mx-1 font-bold text-primary">
        {{ maxSize }}MB
      </div>
      的
      <div class="mx-1 font-bold text-primary">
