...
import { downloadFileFromBlob } from '@/utils/down';
...
<template>
      ...
      <Row type="flex" justify="end">
        <ButtonGroup key="scale-control">
          <Button v-if="!readonly" @click="exportJson">
            <Icon icon="lucide:download" /> 导出
          </Button>
          <Button v-if="!readonly" @click="importJson">
            <Icon icon="lucide:upload" />导入
          </Button>
          <!-- 用于打开本地文件 -->
          <input
            v-if="!readonly"
          ...
          <Button @click="processReZoom()">
            <Icon icon="lucide:table-columns-split" />
          </Button>
  ...
  >
    ...
    <div
      ...
      :key="index"
      class="mb-3 rounded p-2 line-height-normal"
    >
